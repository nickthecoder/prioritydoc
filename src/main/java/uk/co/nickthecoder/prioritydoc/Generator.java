...
public class Generator
{
    ...
    protected Map<String, ClassDoc> knownClassesByQualifiedName;
    ...
    /**
     * If a set of classes is specified, then the old-way of listing the packages doesn't work.
     * So instead, this list is generated by finding the package of each listed class.
     */
    protected List<PackageDoc> packages;

    protected PackageDoc mainPackage;
    ...
    {
        this.knownClassesByQualifiedName = new HashMap<String, ClassDoc>();
        ...
        for (ClassDoc classDoc : this.root.classes()) {
            this.knownClassesByQualifiedName.put(classDoc.qualifiedName(), classDoc);
    ...
    protected void cachePackages()
    {
        if (this.root.specifiedPackages().length == 0) {
            Map<String, PackageDoc> map = new HashMap<String, PackageDoc>();
            for (ClassDoc classDoc : this.root.specifiedClasses()) {
                PackageDoc packageDoc = classDoc.containingPackage();
                map.put(packageDoc.name(), packageDoc);
                if (packageDoc.name().equals(this.options.mainPackage)) {
                    this.mainPackage = packageDoc;
                }
            }
            this.packages = new ArrayList<PackageDoc>();
            this.packages.addAll(map.values());
    ...
    protected void generateDiagrams()
        throws Exception
    {
        for (String filename : this.options.diagrams) {
            generateDiagram(filename);
        }
    }
        ...
        throws Exception
    {
        ...
        System.out.println("Generating diagram : from " + file);
        ...
        Diagram diagram = new DiagramReader().read(file);
        root.put("diagram", diagram);

        for (ClassBox classBox : diagram.classBoxes) {
            ClassDoc classDoc = this.knownClassesByQualifiedName.get(classBox.name);
            if (classDoc == null) {
                throw new Exception("Class " + classBox.name + " not found");
            }
            classBox.setClassDoc(classDoc);
        }

        File destination = new File(this.options.destinationDirectory, diagram.filename);
        generate("diagram.ftl", root, destination);

        if (diagram.imageFilename != null) {
    ...
    public Map<String, Object> createModelRoot()
    {
        ...
        map.put("mainPackage", this.mainPackage);
